function init_lab() {
    var help_slide_number = 0,
        ...
        window = '<div class="vlab_setting"><div class="block_title">' +
            ...
            '<p>Вы находитесь в виртуальной оптической лаборатории, где Вашему вниманию предлагается электронная модель опыта с дифракционной решёткой.' +
            ' Дифракционная решётка - оптический прибор, предназначенный для анализа спектрального состава оптического излучения. Дифракционная решётка состоит из множества узких ' +
            'и близко расположенных щелей. Её определяют следующие характеристики: количество штрихов по оси <i>x</i> <i>N</i><sub><i>x</i></sub>, количество штрихов по оси <i>y</i> <i>N</i><sub><i>y</i></sub>,' +
            ' расстояние между соседними штрихами по оси <i>x</i> <i>d</i><sub><i>x</i></sub>, расстояние между соседними штрихами по оси <i>y</i> <i>d</i><sub><i>y</i></sub>,' +
            ' расстояние между центрами соседних штрихов по оси <i>x</i> &Lambda;<sub><i>x</i></sub>, расстояние между центрами соседних штрихов по оси <i>y</i> &Lambda;<sub><i>y</i></sub>,' +
            ' высота выступа <i>H</i> и показатель преломления <i>n</i>.</p>' +
            '<p>Вы можете менять конфигурацию дифракционной решётки с помощью слайдеров с названиями величин. ' +
            'Кроме этого можно изменить расстояние от решётки до экрана <i>L</i> и длину волны монохроматического некогерентного источника &lambda; (слайдеры расположены под установкой). ' +
            'Для визуализации результатов кликните мышью по пиктограмме, находящейся справа от установки,' +
            ' она запустит обновление экрана с дифракционной картиной. С характерным видом дифракционной картины можно познакомиться,' +
            ' кликнув по левой части переключателя «Образец - Результат». </p>' +
            '<p>В нижней части виртуального стенда расположены графики зависимости интенсивности от координаты по шкале <i>x</i> и <i>y</i>. ' +
            'Находящиеся под ними переключатели «Образец - Результат - Сравнение» позволяют поочередно выводить графики зависимостей для ' +
            'первоначально устанавливаемых при загрузке параметров оптической системы и текущего состояния, а также - оба графика одновременно.</p>' +
            '<p>Желаем успехов в прохождении виртуальной лаборатории!</p>' +
            '</div></div>';
